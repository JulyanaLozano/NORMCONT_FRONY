...
export class UserAppComponent implements OnInit {
  ...
  handlerLogin() {
    this.sharingData.handlerLoginEventEmitter.subscribe(({ codUsuario, password }) => {
      console.log(codUsuario + ' ' + password);

      this.authService.loginUser({ codUsuario, password }).subscribe({
        next: response => {
          const token = response.token;
          console.log(token);
          const payload = this.authService.getPayload(token);

          const user = { codUsuario: payload.sub };
          const login = {
            user,
            isAuth: true,
    ...
    this.sharingData.newUserEventEmitter.subscribe(user => {
      if (user.idUsuario > 0) {
        this.service.update(user).subscribe(
          {
            next: (userUpdated) => {
              this.users = this.users.map(u => (u.idUsuario == userUpdated.idUsuario) ? { ...userUpdated } : u);
